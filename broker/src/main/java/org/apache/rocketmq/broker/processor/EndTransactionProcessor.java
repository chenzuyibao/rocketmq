...
public class EndTransactionProcessor implements NettyRequestProcessor {
    ...
    public RemotingCommand processRequest(ChannelHandlerContext ctx, RemotingCommand request) throws
        ...
        if (MessageSysFlag.TRANSACTION_COMMIT_TYPE == requestHeader.getCommitOrRollback()) {
            // commitMessage提交消息。真实逻辑是根据传过来的commitLogOffset从CommitLog获取half消息
            ...
            if (result.getResponseCode() == ResponseCode.SUCCESS) {
                ...
                if (res.getCode() == ResponseCode.SUCCESS) {
                    ...
                    // 将还原后的消息重新落盘，随后就会调用CommitLogDispatcherBuilderConsumeQueue.dispatch更新原始Topic的ConsumeQueue
                    // 让该消息对消费者可见
                    RemotingCommand sendResult = sendFinalMessage(msgInner);
                    if (sendResult.getCode() == ResponseCode.SUCCESS) {
                        // 删除half消息（并非从磁盘真正的删除）
                        // 具体的逻辑是构建一个消息放入RMQ_SYS_TRANS_OP_HALF_TOPIC队列，queueId为half消息queueId
                        // 消息内容是half消息的queueOffset，并将TAGS属性设置为"d"
            ...
            if (result.getResponseCode() == ResponseCode.SUCCESS) {
                RemotingCommand res = checkPrepareMessage(result.getPrepareMessage(), requestHeader);
                if (res.getCode() == ResponseCode.SUCCESS) {
                    // 删除half消息，具体逻辑是将消息放入RMQ_SYS_TRANS_OP_HALF_TOPIC队列，消息内容为对应half消息的queueOffset，并将TAGS属性设置为"d"
                    this.brokerController.getTransactionalMessageService().deletePrepareMessage(result.getPrepareMessage());
                }
                return res;
            }
